...
addToList() {
    if (this.user) {
      ...
      listRef.forEach(list => {
        console.log(list)
        if (!list)
             this.afs.doc(`notificationList/${Clist}`).set({list: subList});
        else if(!list.list)
            this.afs.doc(`notificationList/${Clist}`).set({list: subList});
            ...
            console.log(list.list)
            return this.afs.doc(`notificationList/${Clist}`).set({list: list.list});
      }
...
DelFromList(i) {
    if (this.user) {
      ...
      if (!list ){
            ...
            console.log(list.list)
            return this.afs.doc(`notificationList/${i}`).set({list: list.list});
      }
      else{console.log("not in this list...")}
